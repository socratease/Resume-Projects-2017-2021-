...
def author():                                                                                             
        return "cgraves36" # replace gburdell3 with your Georgia Tech username.                                                                                             
                                                                                              
def gtid():                                                                                               
    return 903469195 # replace with your GT ID number      
...
class PageRank:
    ...
    def calculate_node_degree(self):
        ...
        #############################################
            ...
            if target in self.node_degree.keys():
                pass
            else:
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        #############################################
#                ranksum = 0
#                #inlist = [(x, y) for (x, y) in self.read_edge_file(self.edge_file) if y == target]
#                #sourcenodes = set([a for a, b in inlist])
#                sourcenodes = set([x for (x,y) in self.read_edge_file(self.edge_file) if y == target])
#                for node in sourcenodes:
#                    ranksum += pr_values[node] / self.node_degree[node]
#                new_pr_values[target] = ((1 - damping_factor) * node_weights[target])
#                new_pr_values[target] += (damping_factor * ranksum)
                #FIRST ATTEMPT: pr = (damping_factor * sum(list(map(np.divide, pr_values, [self.node_degree[key] for key in sorted(self.node_degree.keys())]))))
...
#            new_pr_values = [damping_factor * x for x in new_pr_values]
            for x in range(0, self.max_node_id):
#                if new_pr_values[target] == 0.0:
                 new_pr_values[x] += ((1 - damping_factor) * node_weights[x])
#            new_pr_values = [x + ((1 - damping_factor) / (max_node_id + 1)) for x in new_pr_values]
