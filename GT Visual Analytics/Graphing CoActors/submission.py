...
class Graph:
    ...
    def add_node(self, id: str, name: str)->None:
        ...
        """
        return self.nodes.append((id, name.encode("ascii", "ignore").decode("ascii").replace(",", ""))) if (id, name) not in self.nodes else self.nodes
    ...
    def add_edge(self, source: str, target: str)->None:
        ...
        """
        return self.edges.append((source, target)) if (source, target) not in self.edges and (target, source) not in self.edges else self.edges
        ### Need to account for (a, b) = (b, a)?                         
    ...
    def max_degree_nodes(self)->dict:
        ...
        or {'a': 22, 'b': 22}
        """
        edgelist = [edge for t in self.edges for edge in t]
        degrees = Counter(edgelist)
        degreedic = degrees.most_common()
        val = degreedic[0][1] 
        d = dict(takewhile(lambda x: x[1] >= val, degreedic))
        return d 
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        initurl = "api.themoviedb.org"
        requeststring = "/3/movie/" + movie_id + "/credits?api_key=" + self.api_key + "&language=en-US"
        con = http.client.HTTPConnection(initurl, 80)
        con.connect()
        con.request("GET", requeststring)
        response = con.getresponse()
        dat = response.read()
        datadict = json.loads(dat)
        try:
            cast = datadict['cast']
        except:
            return []
        ##code to exclude peoples
        excludecast = cast if exclude_ids is None else [member for member in cast if member['id'] not in exclude_ids]
                
        ##order items based on "order"
        limitedcast = excludecast if limit is None else [member for member in excludecast if member['order'] < limit]
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        """
        initurl2 = "api.themoviedb.org"
        requeststring2 = "/3/person/" + person_id + "/movie_credits?api_key=" + self.api_key + "&language=en-US"
        con2 = http.client.HTTPConnection(initurl2, 80)
        con2.connect()
        con2.request("GET", requeststring2)
        response2 = con2.getresponse()
        dat2 = response2.read()
        datadict2 = json.loads(dat2)
        try:
            cast2 = datadict2['cast']
        except:
            return
        filteredcast2 = cast2 if vote_avg_threshold is None else [movie for movie in cast2 if movie['vote_average'] >= vote_avg_threshold]
       
        return filteredcast2
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return "https://poloclub.github.io/argo-graph-lite/#eb0000bd-ce86-4451-ab22-0cfc50493018"
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='d05b7ec693f5f24bc59ea58a7dcffb07')
    mscred1 = tmdb_api_utils.get_movie_credits_for_person(person_id='5064', vote_avg_threshold=8.0)
#    print(mscred1[1]['id'])
#    print(tmdb_api_utils.get_movie_cast(movie_id=str(mscred1[2]['id']), limit=5))
    ...
    for m in mscred1:
        cast1 = tmdb_api_utils.get_movie_cast(movie_id=str(m['id']) , limit=3, exclude_ids=r1)
        for c in cast1:
            graph.add_node(id=str(c['id']), name=c['name'])
            graph.add_edge(source='5064', target=str(c['id']))
            if c['id'] not in r1:
                r1.append(c["id"])
        if m['id'] not in ms:
            ms.append(m['id'])
    for person in r1:
        cred2 = tmdb_api_utils.get_movie_credits_for_person(person_id=str(person), vote_avg_threshold=8.0)
        for m2 in cred2:
#            if m2['id'] in ms:
#               continue
#            else:
                cast2 = tmdb_api_utils.get_movie_cast(movie_id=str(m2['id']) , limit=3, exclude_ids=[])
                for c2 in cast2:
                    graph.add_node(id=str(c2['id']), name=c2['name'])
                    graph.add_edge(source=str(person), target=str(c2['id']))
                    if c2['id'] not in r2:
                        r2.append(c2["id"])
                if m2['id'] not in ms:
                    ms.append(m2['id'])
    ...
    for person in r2:
        if person in r1:
            ...
            cred3 = tmdb_api_utils.get_movie_credits_for_person(person_id=str(person), vote_avg_threshold=8.0)
            for m3 in cred3:
#                if m3['id'] in ms:
#                    continue
#                else:
                    cast3 = tmdb_api_utils.get_movie_cast(movie_id=str(m3['id']) , limit=3, exclude_ids=r1)
                    for c3 in cast3:
                        graph.add_node(id=str(c3['id']), name=c3['name'])
                        graph.add_edge(source=str(person), target=str(c3['id']))
                        if c3['id'] not in r3:
                            r3.append(c3['id'])
                    if m3['id'] not in ms:
                        ms.append(m3['id'])
                        ms3.append(m3['id'])
#                        
    graph.edges = [(x, y) for (x, y) in graph.edges if not x == y]
...
#            
#    
#    print(graph.nodes[:min([100, len(graph.nodes)])])
#    print(graph.edges[:min([100, len(graph.edges)])])
#    graph.write_edges_file()
#    graph.write_nodes_file()
#    
#   
##    print(len(r3))
#    
#    credtest = tmdb_api_utils.get_movie_credits_for_person(person_id='5064', vote_avg_threshold=8.0)
#    credtest
#    casttest = tmdb_api_utils.get_movie_cast(movie_id=str(credtest[1]['id']) , limit=3, exclude_ids=[])
#    casttest
#    
#    graph.nodes[:100]
#    print(len(ms))
